<x-layouts.auth title="Masuk" :icon="false">
    ...
    <form action="/login" method="POST">
        ...
        <div class="group mb-3">
            ...
            <input type="text" name="username" id="username" value="{{ old('username') }}" class="px-2 py-2.5 w-full block outline-2 outline-slate-400 rounded focus:outline-sky-600 focus:outline-2 transition text-lg">
            @error('username')
        ...
        <div class="group mb-3">
            ...
            <input type="password" name="password" id="password"
                class="px-2 py-2.5 w-full block outline-2 outline-slate-400 rounded focus:outline-sky-600 focus:outline-2 transition text-lg">
            @error('password')
                <small class="text-red-600">{{ $message }}</small>
            @enderror
        </div>
